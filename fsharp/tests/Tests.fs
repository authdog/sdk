...
let ``GetUserInfo success`` () = task {
    ...
    let handler = new MockHandler(fun _ -> new HttpResponseMessage(HttpStatusCode.OK, Content = new StringContent(body)))
...
let ``GetUserInfo unauthorized`` () = task {
    ...
    use client = new HttpClient(handler)
    let sdk = new AuthdogClient({ BaseUrl = "https://api.authdog.com"; ApiKey = None; TimeoutMs = Some 10000; HttpClient = Some client })
    let! ex = Assert.ThrowsAsync<AuthenticationException>(fun () -> sdk.GetUserInfo("bad").AsTask())
...
let ``GetUserInfo GraphQL error`` () = task {
    ...
    let handler = new MockHandler(fun _ -> new HttpResponseMessage(HttpStatusCode.InternalServerError, Content = new StringContent(body)))
    use client = new HttpClient(handler)
    let sdk = new AuthdogClient({ BaseUrl = "https://api.authdog.com"; ApiKey = None; TimeoutMs = Some 10000; HttpClient = Some client })
    let! ex = Assert.ThrowsAsync<ApiException>(fun () -> sdk.GetUserInfo("t").AsTask())
...
let ``GetUserInfo other http error`` () = task {
    let handler = new MockHandler(fun _ -> new HttpResponseMessage(HttpStatusCode.BadRequest, Content = new StringContent("Bad")))
    use client = new HttpClient(handler)
    let sdk = new AuthdogClient({ BaseUrl = "https://api.authdog.com"; ApiKey = None; TimeoutMs = Some 10000; HttpClient = Some client })
    let! ex = Assert.ThrowsAsync<ApiException>(fun () -> sdk.GetUserInfo("t").AsTask())
